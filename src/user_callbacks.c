...
Eina_List *event_ids = NULL;
Eina_List *todo_ids = NULL;
...
static void calendar_init()
{
    event_ids = NULL;
    todo_ids = NULL;
    int_ok = calendar_connect();

    if (int_ok != CALENDAR_ERROR_NONE) {
        PRINT_MSG("Initialization failed");
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_connect() failed: %d", int_ok);
    }
}
...
static calendar_gl_event_data_t *_create_gl_event_data(calendar_record_h record)
{
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "record get summary() failed: %i", error_code);
        PRINT_MSG("record get summary() failed: %i", error_code);
        _free_gl_event_data(gl_event_data);
        return NULL;
    }

    error_code =
        calendar_record_get_str(record, _calendar_event.description, &gl_event_data->description);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "record get description() failed: %i", error_code);
        PRINT_MSG("record get description() failed: %i", error_code);
        _free_gl_event_data(gl_event_data);
        return NULL;
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "record get id() failed: %i", error_code);
        PRINT_MSG("record get id() failed: %i", error_code);
        _free_gl_event_data(gl_event_data);
        return NULL;
    }

    error_code =
        calendar_record_get_caltime(record, _calendar_event.start_time, &gl_event_data->start_time);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar get start time() failed: %i", error_code);
        PRINT_MSG("calendar get start time() failed: %i", error_code);
        _free_gl_event_data(gl_event_data);
        return NULL;
    }
...
{
    PRINT_MSG("Event change callback called");
}

void _managing_events(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    ...
    PRINT_MSG("Creating an Event");
    error_code = calendar_record_create(_calendar_event._uri, &event);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_create() failed: %d", error_code);
        PRINT_MSG("calendar_record_create: %d", error_code);
    }

    // Setting Event Properties
    const char *summary = "Event summary";
    PRINT_MSG("Summary: %s", summary);
    error_code = calendar_record_set_str(event, _calendar_event.summary, summary);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
    }

    const char *description = "Event description";
    PRINT_MSG("Description: %s", description);
    ...
    PRINT_MSG("(more properties added)");
    ...
    starttime.time.utime = 1404036200;  // 2014/06/29 10:00:00 UTC
    error_code = calendar_record_set_caltime(event, _calendar_event.start_time, starttime);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_caltime() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_caltime() failed: %d", error_code);
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_int() failed: %d", error_code);
    }
    ...
    error_code +=
        calendar_record_set_int(alarm, _calendar_alarm.tick_unit,
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        ...
        PRINT_MSG("calendar add alarm() failed ");
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar add attendee() failed ");
        PRINT_MSG("calendar add alarm() failed ");
    }
    ...
    PRINT_MSG("Inserting an Event");
    int id = -1;
    error_code = calendar_db_insert_record(event, &id);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_insert_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_insert_record() failed: %d", error_code);
    } else {
        PRINT_MSG("Event inserted successfully");

        event_ids = eina_list_append(event_ids, (void *)id);
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_get_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_get_record() failed: %d", error_code);
    }

    calendar_record_destroy(record, true);

    PRINT_MSG("Creating a query");
    calendar_query_h query = NULL;
    error_code = calendar_query_create(_calendar_event._uri, &query);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_query_create() failed: %d", error_code);
        PRINT_MSG("calendar_query_create() failed: %d", error_code);
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_create() failed: %d", error_code);
        PRINT_MSG("calendar_filter_create() failed: %d", error_code);
    }

    const char *summary_pattern = "Event summary";
    error_code =
        calendar_filter_add_str(filter, _calendar_event.summary, CALENDAR_MATCH_STARTSWITH,
                                summary_pattern);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_str() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_str() failed: %d", error_code);
    }

    error_code = calendar_filter_add_operator(filter, CALENDAR_FILTER_OPERATOR_AND);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_operator() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_operator() failed: %d", error_code);
    }

    const char *description_pattern = "Event description";
    error_code =
        calendar_filter_add_str(filter, _calendar_event.description, CALENDAR_MATCH_STARTSWITH,
                                description_pattern);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_str() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_str() failed: %d", error_code);
    ...
    time_to_compare.type = CALENDAR_TIME_UTIME;
    time_to_compare.time.utime = 14000000;

    error_code =
        calendar_filter_add_caltime(filter, _calendar_event.start_time, CALENDAR_MATCH_GREATER_THAN,
                                    time_to_compare);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_caltime() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_caltime() failed: %d", error_code);
    }

    error_code = calendar_query_set_filter(query, filter);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_query_set_filter() failed: %d", error_code);
        PRINT_MSG("calendar_query_set_filter() failed: %d", error_code);
    }

    PRINT_MSG("Filter:");
    PRINT_MSG("Summary starts with: %s", summary_pattern);
    PRINT_MSG("Description starts with: %s", description_pattern);
    PRINT_MSG("Start time after: %lld (utime)", time_to_compare.time.utime);

    PRINT_MSG("Getting Events using the query");

    calendar_list_h cal_list = NULL;
    error_code = calendar_db_get_records_with_query(query, 0, 0, &cal_list);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_db_get_records_with_query() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_get_records_with_query() failed: %d",
                   error_code);
    }

    error_code = calendar_query_destroy(query);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_query_destroy() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_query_destroy() failed: %d", error_code);
    }

    error_code = calendar_filter_destroy(filter);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_filter_destroy() failed: %d", error_code);
        dlog_print(DLOG_DEBUG, LOG_TAG, "calendar_filter_destroy() failed: %d", error_code);
    }

    calendar_gl_event_data_t *gl_event_data = NULL;

    while (calendar_list_get_current_record_p(cal_list, &record) == CALENDAR_ERROR_NONE) {
        gl_event_data = _create_gl_event_data(record);

        // You can get, for example, summary:
        PRINT_MSG("Summary: %s", gl_event_data->summary);

        _free_gl_event_data(gl_event_data);

        error_code = calendar_list_next(cal_list);
        if (error_code != CALENDAR_ERROR_NONE) {
            PRINT_MSG("Getting Events completed");
            break;
        }
    }

    error_code = calendar_list_destroy(cal_list, true);
    cal_list = NULL;

    PRINT_MSG("");
    PRINT_MSG("Updating an Event");

    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_destroy() failed: %d", error_code);
        dlog_print(DLOG_DEBUG, LOG_TAG, "calendar_record_destroy() failed: %d", error_code);
    }
    ...
    error_code = calendar_record_set_str(record, _calendar_event.summary, "Updated event summary");
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
    }
    ...
    error_code =
        calendar_record_set_str(record, _calendar_event.description, "Updated event description");
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_update_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_update_record() failed: %d", error_code);
    } else
        PRINT_MSG("Event successfully updated");

    PRINT_MSG("");

    error_code = calendar_record_destroy(record, true);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_destroy() failed: %d", error_code);
        dlog_print(DLOG_DEBUG, LOG_TAG, "calendar_record_destroy() failed: %d", error_code);
    }
    ...
    error_code += calendar_record_create(_calendar_event._uri, &event);
    error_code += calendar_record_set_str(event, _calendar_event.summary, "A recurring event");
    error_code += calendar_record_set_str(event, _calendar_event.description, "Recurring event description");
    error_code += calendar_record_set_str(event, _calendar_event.start_tzid, "Asia/Seoul");
    error_code += calendar_record_set_str(event, _calendar_event.end_tzid, "Asia/Seoul");

    calendar_time_s st = { 0 };
    st.type = CALENDAR_TIME_UTIME;
    st.time.utime = 1355526000;
    ...
    error_code += calendar_record_set_int(event, _calendar_event.count, 8);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: ");
        PRINT_MSG("calendar_record_set_int() failed: ");
    }
    ...
    int m_event_id = -1;
    PRINT_MSG("Inserting a recurring Event");
    error_code = calendar_db_insert_record(event, &m_event_id);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_insert_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_insert_record() failed: %d", error_code);
    } else {
        PRINT_MSG("Recurring Event inserted");

        event_ids = eina_list_append(event_ids, (void *)m_event_id);
    }
    ...
    // The following line "connects" events, so only one has to be removed later
    ...
    int exdate_event_id = -1;

    PRINT_MSG("Inserting a modified recurring Event");
    error_code = calendar_db_insert_record(clone, &exdate_event_id);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_insert_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_insert_record() failed: %d", error_code);
    } else
        PRINT_MSG("Modified recurring Event inserted");
...
static calendar_gl_todo_data_t *_create_gl_todo_data(calendar_record_h record)
{
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_get_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_get_str() failed: %d", error_code);
        _free_gl_todo_data(gl_todo_data);
        return NULL;
    }

    error_code =
        calendar_record_get_str(record, _calendar_todo.description, &gl_todo_data->description);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_get_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_get_str() failed: %d", error_code);
        _free_gl_todo_data(gl_todo_data);
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_get_int() failed: %d", error_code);
        PRINT_MSG("calendar_record_get_int() failed: %d", error_code);
        _free_gl_todo_data(gl_todo_data);
        return NULL;
    }

    error_code =
        calendar_record_get_caltime(record, _calendar_todo.due_time, &gl_todo_data->due_time);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_get_caltime() failed: %d", error_code);
        PRINT_MSG("calendar_record_get_caltime() failed: %d", error_code);
        _free_gl_todo_data(gl_todo_data);
        return NULL;
    }
...
{
    PRINT_MSG("Todo change callback called");
}
...
void calendar_cleanup(bool print_on_screen)
{
    ...
    // Remove all created events and Todos
    if (print_on_screen) {
        PRINT_MSG("Removing all added events and Todos");
    }

    const Eina_List *l;
    void *element_id = NULL;
    if (event_ids != NULL) {
        EINA_LIST_FOREACH(event_ids, l, element_id) {
            error_code = calendar_db_delete_record(_calendar_event._uri, (int)element_id);
            if (error_code != CALENDAR_ERROR_NONE) {
                dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_delete_record() failed: %d", error_code);
                if (print_on_screen)
                    PRINT_MSG("calendar_db_delete_record() failed: %d", error_code);
            }
        }
        eina_list_free(event_ids);
        event_ids = NULL;
    }

    if (todo_ids != NULL) {
        EINA_LIST_FOREACH(todo_ids, l, element_id) {
            error_code = calendar_db_delete_record(_calendar_todo._uri, (int)element_id);
            if (error_code != CALENDAR_ERROR_NONE) {
                dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_delete_record() failed: %d", error_code);
                if (print_on_screen)
                    PRINT_MSG("calendar_db_delete_record() failed: %d", error_code);
        ...
        eina_list_free(todo_ids);
        todo_ids = NULL;
    }
}

void _remove_events_todos(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    if (int_ok != CALENDAR_ERROR_NONE) {
        PRINT_MSG("Initialization failed");
        return;
    ...
    // Remove all added events/Todos, print messages on screen and to logs
    calendar_cleanup(true);
}

void _add_change_callback(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    int error_code;
    ...
    if (ad->callbacks_set) {
        PRINT_MSG("Callbacks already set");
        return;
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "db add changed cb() failed: %d", error_code);
        PRINT_MSG("db add changed cb() failed: %d", error_code);
    } else {
        dlog_print(DLOG_DEBUG, LOG_TAG, "Event change callback set");
        PRINT_MSG("Event change callback set");
    }

    // Monitoring Todo Changes
    error_code = calendar_db_add_changed_cb(_calendar_todo._uri, _todo_changed_callback, NULL);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_add_changed_cb() failed: %d", error_code);
        PRINT_MSG("calendar_db_add_changed_cb() failed: %d", error_code);
    } else {
        dlog_print(DLOG_DEBUG, LOG_TAG, "Todo change callback set");
        PRINT_MSG("Todo change callback set");
    }

    PRINT_MSG("");

    ad->callbacks_set = true;
}

void _remove_change_callback(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    int error_code;
    ...
    error_code = calendar_db_remove_changed_cb(_calendar_event._uri, _event_changed_callback, NULL);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "db remove changed cb() failed: %d", error_code);
        PRINT_MSG("db remove changed cb() failed: %d", error_code);
    } else {
        dlog_print(DLOG_DEBUG, LOG_TAG, "Event change callback removed");
        PRINT_MSG("Event change callback removed");
    }

    error_code = calendar_db_remove_changed_cb(_calendar_todo._uri, _todo_changed_callback, NULL);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_remove_changed_cb() failed: %d", error_code);
        PRINT_MSG("calendar_db_remove_changed_cb() failed: %d", error_code);
    } else {
        dlog_print(DLOG_DEBUG, LOG_TAG, "Todo change callback removed");
        PRINT_MSG("Todo change callback removed");
    }

    PRINT_MSG("");

    ad->callbacks_set = false;;
}
...
void _calender_page(appdata_s *ad)
{
    // Setting the window
	...
	// Create a naviframe
	    ...
	    create_buttons_in_main_window3(ad);
...
void _hello(appdata_s *ad, Evas_Object *obj, void *event_info)
{
...
void _contacts_management(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ad->navi12 = elm_naviframe_add(conform);
			    evas_object_size_hint_align_set(ad->navi12, EVAS_HINT_FILL, EVAS_HINT_FILL);
			    evas_object_size_hint_weight_set(ad->navi12, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

			    elm_object_content_set(conform, ad->navi12);
			    evas_object_show(ad->navi12);

			    // Fill the list with items
	create_buttons_in_main_window12(ad,obj,event_info);


			    eext_object_event_callback_add(ad->navi12, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _groups_management(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ad->navi13 = elm_naviframe_add(conform);
			    evas_object_size_hint_align_set(ad->navi13, EVAS_HINT_FILL, EVAS_HINT_FILL);
			    evas_object_size_hint_weight_set(ad->navi13, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

			    elm_object_content_set(conform, ad->navi13);
			    evas_object_show(ad->navi13);

			    // Fill the list with items
	create_buttons_in_main_window13(ad,obj,event_info);


			    eext_object_event_callback_add(ad->navi13, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _vcards_management(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ...
			    // Fill the list with items
	create_buttons_in_main_window14(ad,obj,event_info);
...
void _phone_logs(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ...
			    // Fill the list with items
	create_buttons_in_main_window15(ad,obj,event_info);
...
void _speed_dials(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ...
			    // Fill the list with items
	create_buttons_in_main_window16(ad,obj,event_info);
...
void _sim_contacts(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ...
			    // Fill the list with items
	create_buttons_in_main_window17(ad,obj,event_info);
...
void _contacts_settings(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	...
	// Create a naviframe
	    ...
	    // Fill the list with items
	    create_buttons_in_main_window18(ad,obj,event_info);
...
static void _bluetooth_bond_device(appdata_s *ad, void *data, Evas_Object *obj, void *event_info)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ad->navi3 = elm_naviframe_add(conform);
			    evas_object_size_hint_align_set(ad->navi3, EVAS_HINT_FILL, EVAS_HINT_FILL);
			    evas_object_size_hint_weight_set(ad->navi3, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

			    elm_object_content_set(conform, ad->navi3);
			    evas_object_show(ad->navi3);

			    // Fill the list with items
	create_buttons_in_main_window4(ad,data,obj,event_info);


			    eext_object_event_callback_add(ad->navi3, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _bluetooth_serial_port_cb(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	// Setting the window
			...
			// Create a naviframe
			    ad->navi4 = elm_naviframe_add(conform);
			    evas_object_size_hint_align_set(ad->navi4, EVAS_HINT_FILL, EVAS_HINT_FILL);
			    evas_object_size_hint_weight_set(ad->navi4, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

			    elm_object_content_set(conform, ad->navi4);
			    evas_object_show(ad->navi4);

			    // Fill the list with items
	create_buttons_in_main_window5(ad,obj,event_info);


			    eext_object_event_callback_add(ad->navi4, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _bluetooth_opp_cb(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	...
	// Create a naviframe
	    ad->navi5 = elm_naviframe_add(conform);
	    evas_object_size_hint_align_set(ad->navi5, EVAS_HINT_FILL, EVAS_HINT_FILL);
	    evas_object_size_hint_weight_set(ad->navi5, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

	    elm_object_content_set(conform, ad->navi5);
	    evas_object_show(ad->navi5);

	    // Fill the list with items
	  create_buttons_in_main_window6(ad,obj,event_info);


	    eext_object_event_callback_add(ad->navi5, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _bluetooth_le_cb(appdata_s *ad, Evas_Object *obj, void *event_info)
{
	...
	// Create a naviframe
	    ad->navi7 = elm_naviframe_add(conform);
	    evas_object_size_hint_align_set(ad->navi7, EVAS_HINT_FILL, EVAS_HINT_FILL);
	    evas_object_size_hint_weight_set(ad->navi2, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

	    elm_object_content_set(conform, ad->navi7);
	    evas_object_show(ad->navi7);

	    // Fill the list with items
	  create_buttons_in_main_window8(ad,obj,event_info);


	    eext_object_event_callback_add(ad->navi7, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _bluetooth_errorcodes(appdata_s *ad)
{
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ad->navi9 = elm_naviframe_add(conform);
			    evas_object_size_hint_align_set(ad->navi9, EVAS_HINT_FILL, EVAS_HINT_FILL);
			    evas_object_size_hint_weight_set(ad->navi9, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

			    elm_object_content_set(conform, ad->navi9);
			    evas_object_show(ad->navi9);

			    // Fill the list with items
	create_buttons_in_main_window9(ad);


			    eext_object_event_callback_add(ad->navi9, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void _bt_socket_send_data (appdata_s *ad, char *data, Evas_Object *obj, void *event_info){
	// Setting the window
		    ad->win = elm_win_util_standard_add(PACKAGE, PACKAGE);
		    elm_win_conformant_set(ad->win, EINA_TRUE);
		    elm_win_autodel_set(ad->win, EINA_TRUE);
			...
			// Create a naviframe
			    ad->navi8 = elm_naviframe_add(conform);
			    evas_object_size_hint_align_set(ad->navi8, EVAS_HINT_FILL, EVAS_HINT_FILL);
			    evas_object_size_hint_weight_set(ad->navi8, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

			    elm_object_content_set(conform, ad->navi8);
			    evas_object_show(ad->navi8);

			    // Fill the list with items
	create_buttons_in_main_window10(ad,data,obj,event_info);


			    eext_object_event_callback_add(ad->navi8, EEXT_CALLBACK_BACK, eext_naviframe_back_cb, NULL);
...
void create_buttons_in_main_window(appdata_s *ad)
{
	 Evas_Object *display = _create_new_cd_display(ad, "Main", _pop_cb, ad->navi);
	 	...
	 	//_new_button(ad, display, "Contacts", _contacts_page);
	 	    _new_button(ad, display, "Calendar", _calender_page);
}
...
void create_buttons_in_main_window1(appdata_s *ad)
{
    ...
    _new_button(ad, display, "Bond", _bluetooth_bond_device);  //done window4
   // _new_button(ad, display, "Serial Port Profile", _bluetooth_serial_port_cb); //done window5
    //_new_button(ad, display, "Object Push profile", _bluetooth_opp_cb); // done window6
    //_new_button(ad, display, "GATT operations", _bluetooth_gatt_cb); //done window7
    //_new_button(ad, display, "LE scans", _bluetooth_le_cb); //done window8
    //_new_button(ad, display, "Send Data", _bt_socket_send_data);// done window10
    _new_button(ad, display, "    ", _hello);  //done window4

    //_new_button(ad, display, "Error Codes", _bluetooth_errorcodes); // done window9
...
create_buttons_in_main_window2(Evas_Object *display, appdata_s *ad){
	...
	//Evas_Object *display = _create_new_cd_display(ad, "Contacts", _pop_cb,ad->navi2);
		contacts_init();
	    ...
	    _new_button(ad, display, "Managing contacts", _contacts_management);
	     _new_button(ad, display, "Managing groups", _groups_management);
	     _new_button(ad, display, "Managing vCards", _vcards_management);
	     _new_button(ad, display, "Phone logs", _phone_logs);
	     _new_button(ad, display, "Speed dials", _speed_dials);
	     _new_button(ad, display, "Managing SIM contacts", _sim_contacts);
	    _new_button(ad, display, "Managing contacts settings", _contacts_settings);

}

void create_buttons_in_main_window3(appdata_s *ad)
{
    Evas_Object *display = _create_new_cd_display(ad, "Calendar", _pop_cb,ad->navi1);
    calendar_init();
    _new_button(ad, display, "Managing Events", _managing_events);
    _new_button(ad, display, "Remove added Events/Todos", _remove_events_todos);
    _new_button(ad, display, "Set Events/Todos change callback", _add_change_callback);
    _new_button(ad, display, "Remove Events/Todos change callback", _remove_change_callback);
